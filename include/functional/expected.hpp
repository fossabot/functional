...
template <typename T>
concept some_expected_pack = some_expected<T> && some_pack<typename std::remove_cvref_t<T>::value_type>;
...
template <typename T, typename Err> struct expected final : std::expected<T, Err> {
  ...
  template <typename Fn>
    requires std::is_constructible_v<Err, Err const &>
  ...
  template <typename Fn>
    requires std::is_constructible_v<Err, Err &>
  constexpr auto and_then(Fn &&fn) & requires(not std::same_as<T, void>) && (not some_pack<value_type>)
  {
  ...
  constexpr auto or_else(Fn &&fn) & requires (not std::same_as<T, void>)
  {
  ...
  constexpr auto or_else(Fn &&fn) const & requires (not std::same_as<T, void>)
  {
...
// Tricky to implement, since lh needs to be pre-pended to rh. Probably not useful anyway.
template <some_expected_non_void Lh, some_expected_non_void Rh>
  requires std::same_as<typename std::remove_cvref_t<Lh>::error_type, typename std::remove_cvref_t<Rh>::error_type>
               && (not some_pack<typename std::remove_cvref_t<Lh>::value_type>)
