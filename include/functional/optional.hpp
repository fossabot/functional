...
template <typename T>
concept some_optional_pack = some_optional<T> && some_pack<typename std::remove_cvref_t<T>::value_type>;

template <typename T>
concept some_optional_non_pack = some_optional<T> && !some_pack<typename std::remove_cvref_t<T>::value_type>;
...
constexpr auto operator&(Lh &&lh, Rh &&rh) noexcept
{
  ...
  if (lh.has_value() && rh.has_value())
    return type{std::in_place, pack<lh_type>{FWD(lh).value()}.append(fn::type<rh_type>(), FWD(rh).value())};
  else
    return type{std::nullopt};
}
...
constexpr auto operator&(Lh &&lh, Rh &&rh) noexcept
{
  ...
  using type = optional<value_type>;
  if (lh.has_value() && rh.has_value())
    return type{std::in_place, FWD(lh).value().append(fn::type<rh_type>(), FWD(rh).value())};
  else
    return type{std::nullopt};
}
